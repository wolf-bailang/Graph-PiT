...
########################################################################
# from graph.graph_model import GraphAggregator
from graph.graph_model_H import GraphAggregator
########################################################################
...
class DiT_Llama(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        ########################################################################
        self.graph_agg = GraphAggregator(dim=embedding_dim, num_layers=2, cfg=self.cfg)
        ########################################################################

    # def forward(self, x, t, cond, node_num):
    def forward(self, x, t, cond, Adj):
        self.freqs_cis = self.freqs_cis.to(x.device)

        ########################################################################
        # 如果给定邻接矩阵，先做图消息传递
        # print(f"A shape: {A.shape}")    # torch.Size([1, 4, 4])
        # print(f"cond shape: {cond.shape}")    # torch.Size([1, 32, 2048])
        # print(f"x shape: {x.shape}")    # torch.Size([1, 16, 2048])

        if Adj is not None:
            self.graph_agg = self.graph_agg.to(x.device)
            parts_emb, loss_graph = self.graph_agg(cond, Adj)
            cond = parts_emb.to(x.device)
        ########################################################################, node_num
        x0 = torch.cat([x, cond], dim=1)
        x = x0.to(x.device)
        self.x_embedder = self.x_embedder.to(x.device)
        x = self.x_embedder(x)
    ...
    # def forward_with_cfg(self, x, t, cond, node_num, cfg_scale):
    def forward_with_cfg(self, x, t, cond, Adj, cfg_scale):
        half = x[: len(x) // 2]
        ...
        if Adj is not None:
            ...
            cond = parts_emb
        ########################################################################, node_num
        model_out = self.forward(combined, t, cond, Adj)
        ...
        return torch.cat([eps, rest], dim=1), loss_graph
        ########################################################################
